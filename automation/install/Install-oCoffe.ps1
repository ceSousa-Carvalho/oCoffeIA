...
$config = [ordered]@{
    ...
    slaDiasHistorico = if ($existingConfig.slaDiasHistorico) { [int]$existingConfig.slaDiasHistorico } else { 27 }
    slaHorario = if ($existingConfig.slaHorario) { [string]$existingConfig.slaHorario } else { '07:00' }
    slaPaginaPowerBi = if ($existingConfig.slaPaginaPowerBi) { [string]$existingConfig.slaPaginaPowerBi } else { 'SLA - VENCIMENTO' }
